...
public class RouteService extends Service implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
    ...
    public static final String ACTION = "com.blueobject.app.alive.RouteService";
    ...
    protected LocationRequest createLocationRequest() {
        ...
        mLocationRequest.setInterval(60000*15); // 5 perc
        mLocationRequest.setFastestInterval(30000); // 30 sec max.
    ...
    public void onLocationChanged(Location location) {
        ...
        if(isRouteTrackingOn() && startTimeInMilliSeconds > 0) {
            ...
            Log.w(TAG, "TICK :" + Global.sendTracking);

            Global.user.lat = "" + latitudeValue;
            Global.user.lng = "" + longitudeValue;

            if(Global.sendTracking) {
                new Global.Tracking("" + latitudeValue, "" + longitudeValue).execute();
            }
    ...
    private boolean isRouteTrackingOn(){
        Log.d(TAG, "SERVICE STATE " + Global.shared.getServiceState());
        return Global.shared.getServiceState();
    }
    @Override
