...
import static com.blueobject.app.alive.R.layout.dialog;
...
public abstract class PickImageBaseDialog extends DialogFragment implements IPickClick {
    ...
    private void onSetup() {
        if (setup.getBackgroundColor() != android.R.color.white) {
            ...
            if (showCamera)
                Util.background(tvCamera, Util.getAdaptiveRippleDrawable(Color.parseColor("#dcdcdc")));

            if (showGallery)
                Util.background(tvGallery, Util.getAdaptiveRippleDrawable(Color.parseColor("#dcdcdc")));
        }
